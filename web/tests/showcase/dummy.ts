	...
	return {
		results: DUMMY_DEVICE_GROUPS.map((g) => ({
			...
			fields: {
				name: g.name,
				description: g.description,
				is_dynamic: String(g.dynamic_query !== undefined),
	...
	return {
		results: DUMMY_USER_GROUPS.map((g) => ({
			...
			fields: {
				...
				description: g.description,
				kind: g.kind,
				is_dynamic: String(g.kind === 'dynamic'),
				dynamic_query: g.kind === 'dynamic' ? '(user.disabled equals "false")' : undefined,
...
export function getUserGroupResponse(id: string) {
	...
	return {
		group: {
			...
			description: g.description,
			memberCount: parseInt(g.member_count, 10),
			dynamicQuery: g.kind === 'dynamic' ? '(user.disabled equals "false")' : undefined,
			createdAt: isoFromSec(g.created_at),
			roleGrants: [{
