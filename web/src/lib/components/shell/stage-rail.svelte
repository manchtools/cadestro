<script lang="ts">
	...
	import { SquareTerminal, X, ChevronDown, AppWindow, Pencil } from '@lucide/svelte';
	...
	const terminalStashed = $derived(shell.terminal.sessions.length > 0 && !shell.terminal.open);
...
{#if anything}
	...
	>
		...
		{#if terminalStashed}
			<button type="button" onclick={toggleTerminal} transition:fly={{ x: 24, duration: 160 }} class="flex items-center gap-2 rounded-lg border bg-card p-2 text-left shadow-plate transition-transform hover:scale-[1.03] hover:border-primary">
				<span class="grid h-8 w-8 shrink-0 place-items-center rounded-md bg-foreground text-background"><SquareTerminal class="h-4 w-4" /></span>
				<span class="min-w-0 flex-1">
					<span class="block truncate text-xs font-medium">{m.terminal_title()}</span>
					<span class="text-[10px] text-muted-foreground">{m.stage_terminal_open_count({ count: shell.terminal.sessions.length })}</span>
				</span>
			</button>
		{/if}
