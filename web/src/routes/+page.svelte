	...
	async function load() {
		...
		try {
			...
			const [devicePage, actionPage, groupPage, assignmentPage, tokenPage, userPage, rolePage, permissionPage] = await Promise.all([
				...
				can(Permission.LIST_TOKENS) ? api.listTokens(create(ListTokensRequestSchema, { pageSize: 100, includeDisabled: true })) : null,
...
<header>
	...
	{#if canAny(Permission.LIST_USERS, Permission.LIST_ROLES, Permission.LIST_PERMISSIONS, Permission.CREATE_ROLE, Permission.UPDATE_ROLE, Permission.DELETE_ROLE, Permission.ASSIGN_ROLE_TO_USER, Permission.REVOKE_ROLE_FROM_USER, Permission.REVOKE_USER_SESSIONS)}
		<section class="card">
			...
			{#if can(Permission.LIST_ROLES)}<ul class="compact">{#each roles as role (role.id?.value)}<li><span>{role.name}</span><small>{role.permissions.length} permissions</small>{#if !role.isSystem && can(Permission.UPDATE_ROLE) && can(Permission.LIST_PERMISSIONS)}<button class="quiet" onclick={() => { editingRole = role; roleName = role.name; roleDescription = role.description; rolePermissions = [...role.permissions]; }}>Edit</button>{/if}{#if !role.isSystem && can(Permission.DELETE_ROLE)}<button class="danger" onclick={() => deleteRole(role)} disabled={busy}>Delete</button>{/if}</li>{/each}</ul>{/if}
