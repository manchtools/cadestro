...
export function deviceTone(d: Device): FleetTone {
	...
	if (d.complianceStatus === ComplianceStatus.NON_COMPLIANT) {
		return 'warn';
	}
	return 'ok';
}
...
export function toFleetDevice(d: Device, nowSec: number): FleetDevice {
	const lastSeenSec = seconds(d.lastSeenAt);
	return {
		id: (d.id?.value ?? ''),
		hostname: d.hostname,
		tone: deviceTone(d),
		age: ageBucket(lastSeenSec, nowSec, DEFAULT_SYNC_MINUTES),
		lastSeenSec,
		device: d
	};
}
