...
def device_specific_manifest_compile_matches(root: Path) -> list[Match]:
    """Count the per-device compiler API left behind by field sealing."""
    result: list[Match] = []
    pattern = re.compile(r"\b(?:Action|ActionSet|Definition)ForDevice\b")
    for relative in (
        "server/internal/manifest/compiler.go",
        "server/internal/dispatch/assigned.go",
...
def metric_matches(root: Path) -> dict[str, list[Match]]:
    return {
        ...
        "device_specific_manifest_compile_paths": device_specific_manifest_compile_matches(root),
...
def simplification_report(root: Path, baseline_counts: dict[str, int]) -> dict[str, object]:
    ...
    zero = {
        ...
        "device_specific_manifest_compile_paths": current["device_specific_manifest_compile_paths"] == 0,
