...
func TestPacman_Install(t *testing.T) {
	...
	t.Run("pinned version keeps --needed", func(t *testing.T) {
		m, f := pacmanM(t)
		ok(f, "")
	...
	t.Run("multiple package versions", func(t *testing.T) {
		...
		if got := argv(f.Calls()[0]); got != "pacman -S --noconfirm --needed vim=1.0 git=2.0" {
			t.Fatalf("argv=%q", got)
		}
	})
...
func TestPacman_UpdateUpgrade(t *testing.T) {
	ctx := context.Background()
	t.Run("update is unsupported", func(t *testing.T) {
		m, f := pacmanM(t)
		if _, err := m.Update(ctx); !errors.Is(err, ErrUnsupported) {
			t.Fatalf("err=%v, want ErrUnsupported", err)
		}
		if len(f.Calls()) != 0 {
			t.Fatalf("unsupported update ran %d commands", len(f.Calls()))
		}
	})
	t.Run("UpgradeAll -Syu", func(t *testing.T) {
		...
		c := f.Calls()[0]
		if c.Name != "pacman" || len(c.Args) != 2 || c.Args[0] != "-"+"Syu" || c.Args[1] != "--noconfirm" {
			t.Errorf("argv=%q", argv(f.Calls()[0]))
		}
	})
...
func TestPacman_Show(t *testing.T) {
	...
	t.Run("installed via -Qi", func(t *testing.T) {
		m, f := pacmanM(t)
		f.Push(sysexec.Result{ExitCode: 0, Stdout: installedInfo}, nil)
	...
	t.Run("-Si runner error propagates", func(t *testing.T) {
		m, f := pacmanM(t)
		f.Push(sysexec.Result{ExitCode: 1}, nil)
		f.Push(sysexec.Result{}, errors.New("db corrupt"))
...
func TestPacman_ListVersions(t *testing.T) {
	...
	t.Run("installed-version runner failure propagates", func(t *testing.T) {
		m, f := pacmanM(t)
		f.Push(sysexec.Result{}, errors.New("pacman -Q failed"))
	...
	t.Run("-Si runner failure propagates", func(t *testing.T) {
		m, f := pacmanM(t)
		ok(f, "vim 9.0-1\n")
		f.Push(sysexec.Result{}, errors.New("db"))
		if _, err := m.ListVersions(ctx, "vim"); err == nil {
			t.Fatal("a -Si runner failure must propagate")
		}
	})
...
func TestPacman_Pin(t *testing.T) {
	...
	t.Run("config-injection name rejected by stricter gate", func(t *testing.T) {
		...
		if _, err := m.Pin(ctx, "vim:amd64"); !errors.Is(err, ErrInvalidArgument) || len(f.Calls()) != 0 {
...
func TestPacman_ListPinnedAndIsPinned(t *testing.T) {
	...
	t.Run("ListPinned with versions", func(t *testing.T) {
		...
		if len(pkgs) != 2 || pkgs[0].Name != "linux" || pkgs[0].Version != "6.1-1" {
			t.Fatalf("pkgs=%+v", pkgs)
		}
	})
...
func TestPacman_EnrichmentRunnerFailuresPropagate(t *testing.T) {
	...
	t.Run("ListUpgradable: InstalledVersion runner failure", func(t *testing.T) {
		m, f := pacmanM(t)
		ok(f, "git 2.39-1\n")
		f.Push(sysexec.Result{}, errors.New("-Q"))
		if _, err := m.ListUpgradable(ctx); err == nil {
