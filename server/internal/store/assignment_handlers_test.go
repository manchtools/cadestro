...
type assignmentHandlerFixture struct {
	*actionHandlerFixture
	...
	sources  map[cadestrov1.AssignmentSourceType]string
	targets  map[cadestrov1.AssignmentTargetType]string
	systemID string
}
...
func newAssignmentHandlerFixture(t *testing.T) *assignmentHandlerFixture {
	...
	f := &assignmentHandlerFixture{
		...
		sources: make(map[cadestrov1.AssignmentSourceType]string),
		targets: make(map[cadestrov1.AssignmentTargetType]string),
	}
	...
	f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION] = normal
	...
	f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION_SET] = set.ID
	...
	require.NoError(t, err)
	f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_DEFINITION] = definition.ID
	...
	require.NoError(t, err)
	f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_COMPLIANCE_POLICY] = policy.ID
	...
	system, err := state.CreateAction(context.Background(), op, authoring.CreateActionParams{
		...
		Type: cadestrov1.ActionType_ACTION_TYPE_SHELL, DesiredState: cadestrov1.DesiredState_DESIRED_STATE_PRESENT,
		Params: []byte(`{"interpreter":"/bin/sh","script":"true"}`),
	})
	require.NoError(t, err)
	f.systemID = system.ID

	for _, targetType := range []cadestrov1.AssignmentTargetType{
		cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE,
		cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE_GROUP,
		cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER,
		cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER_GROUP,
	} {
	...
	_, err = f.raw.Exec(ctx, `INSERT INTO devices (id, hostname)
		...
		f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE])
	require.NoError(t, err)
	_, err = f.raw.Exec(ctx, `INSERT INTO device_groups (id, name) VALUES ($1, 'assigned-device-group')`,
		f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE_GROUP])
	require.NoError(t, err)
	_, err = f.raw.Exec(ctx, `INSERT INTO users (id, email, display_name, linux_username, linux_uid)
		...
		f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER])
	require.NoError(t, err)
	_, err = f.raw.Exec(ctx, `INSERT INTO user_groups (id, name, created_by) VALUES ($1, 'assigned-user-group', $2)`,
		f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER_GROUP], f.actorID)
	require.NoError(t, err)
	return f
}
...
func TestAssignmentHandlers_CRUDAcrossEverySourceAndTarget(t *testing.T) {
	...
	types := []struct {
		source cadestrov1.AssignmentSourceType
		target cadestrov1.AssignmentTargetType
	}{
		{cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION, cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE},
		{cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION_SET, cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE_GROUP},
		{cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_DEFINITION, cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER},
		{cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_COMPLIANCE_POLICY, cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER_GROUP},
	}
	created := make([]*cadestrov1.Assignment, 0, len(types))
	searchScopes := map[cadestrov1.AssignmentSourceType]string{
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION:            "actions",
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION_SET:        "action_sets",
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_DEFINITION:        "definitions",
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_COMPLIANCE_POLICY: "compliance_policies",
	}
	...
	filtered, err := f.handlers.ListAssignments(ctx, connect.NewRequest(&cadestrov1.ListAssignmentsRequest{
...
func TestAssignmentHandlers_GetUserAssignmentsResolvesDirectAndGroupTargets(t *testing.T) {
	...
	userID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER]
	groupID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER_GROUP]
...
func TestAssignmentHandlers_AvailableSelectionIsAuditedDirectState(t *testing.T) {
	...
	deviceID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE]
	actionID := f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION]
...
func TestAssignmentHandlers_AvailableSourcesResolveEveryTargetKind(t *testing.T) {
	...
	deviceID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE]
	deviceGroupID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE_GROUP]
	userID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER]
	userGroupID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_USER_GROUP]
...
func TestAssignmentHandlers_AvailableSourceIsHiddenByStrongerMode(t *testing.T) {
	...
	ctx := f.actor("CreateAssignment", "ListAvailableActions", "ListDevices")
	deviceID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE]
	groupID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE_GROUP]
	actionID := f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION]
...
func TestAssignmentHandlers_GetDeviceAssignmentsExpandsLiveSources(t *testing.T) {
	...
	deviceID := f.targets[cadestrov1.AssignmentTargetType_ASSIGNMENT_TARGET_TYPE_DEVICE]
	actionID := f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION]
	setID := f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION_SET]
	definitionID := f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_DEFINITION]
	policyID := f.sources[cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_COMPLIANCE_POLICY]
	...
	for _, source := range []cadestrov1.AssignmentSourceType{
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION,
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_ACTION_SET,
		cadestrov1.AssignmentSourceType_ASSIGNMENT_SOURCE_TYPE_DEFINITION,
	...
	for id, mode := range map[string]cadestrov1.AssignmentMode{
		availableID: cadestrov1.AssignmentMode_ASSIGNMENT_MODE_AVAILABLE,
		excludedID:  cadestrov1.AssignmentMode_ASSIGNMENT_MODE_EXCLUDED,
