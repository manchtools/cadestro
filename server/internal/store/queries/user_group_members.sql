...
INSERT INTO user_group_members (group_id, user_id, added_at, added_by)
SELECT sqlc.arg(group_id), CAST(wanted.value AS TEXT), sqlc.arg(added_at), sqlc.arg(added_by)
FROM json_each(sqlc.arg(user_ids_json)) AS wanted
...
WHERE group_id = sqlc.arg(group_id)
  AND user_id IN (
      SELECT CAST(value AS TEXT) FROM json_each(sqlc.arg(user_ids_json))
  )
  AND EXISTS (
      SELECT 1 FROM user_groups g
      WHERE g.id = user_group_members.group_id
