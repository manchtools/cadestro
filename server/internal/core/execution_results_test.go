...
func TestExecutionResultPersistsSerializedPayload(t *testing.T) {
	...
	result := &cadestrov1.ActionResult{
		...
		Duration: durationpb.New(3 * time.Second), Changed: true, Compliant: true,
...
func TestDeviceComplianceUsesLinkedShellAction(t *testing.T) {
	...
	for _, result := range []*cadestrov1.ActionResult{
		{RunId: &cadestrov1.RunId{Value: "01K00000000000000000000014"}, ActionId: &cadestrov1.ActionId{Value: complianceID}, CompletedAt: timestamppb.New(now), Compliant: false},
		{RunId: &cadestrov1.RunId{Value: "01K00000000000000000000015"}, ActionId: &cadestrov1.ActionId{Value: normalID}, CompletedAt: timestamppb.New(now), Compliant: true},
	} {
