...
func testUser(t *testing.T, service *Service, ctx context.Context, id string) *db.User {
	t.Helper()
	user, err := service.store.Queries().CreateUser(ctx, db.CreateUserParams{ID: id, Email: id + "@example.com", DisplayName: id, LastLoginAt: time.Now().UTC()})
	require.NoError(t, err)
	return user
}
...
func TestRoleMutationsRotateAffectedSessions(t *testing.T) {
	...
	granted, err := service.GrantRolePermission(ctx, connect.NewRequest(&cadestrov1.GrantRolePermissionRequest{Id: custom.Msg.Role.Id, Permission: cadestrov1.Permission_PERMISSION_GET_CURRENT_USER}))
	require.NoError(t, err)
	require.ElementsMatch(t, []cadestrov1.Permission{cadestrov1.Permission_PERMISSION_GET_CURRENT_USER, cadestrov1.Permission_PERMISSION_LIST_USERS}, granted.Msg.Role.Permissions)
	require.EqualValues(t, 3, mustUser(t, service, ctx, target.ID).SessionVersion)
	_, err = service.GrantRolePermission(ctx, connect.NewRequest(&cadestrov1.GrantRolePermissionRequest{Id: custom.Msg.Role.Id, Permission: cadestrov1.Permission_PERMISSION_GET_CURRENT_USER}))
	require.Equal(t, connect.CodeAlreadyExists, connect.CodeOf(err))
	require.EqualValues(t, 3, mustUser(t, service, ctx, target.ID).SessionVersion)
	revoked, err := service.RevokeRolePermission(ctx, connect.NewRequest(&cadestrov1.RevokeRolePermissionRequest{Id: custom.Msg.Role.Id, Permission: cadestrov1.Permission_PERMISSION_LIST_USERS}))
	require.NoError(t, err)
	require.Equal(t, []cadestrov1.Permission{cadestrov1.Permission_PERMISSION_GET_CURRENT_USER}, revoked.Msg.Role.Permissions)
	require.EqualValues(t, 4, mustUser(t, service, ctx, target.ID).SessionVersion)
	_, err = service.RevokeRolePermission(ctx, connect.NewRequest(&cadestrov1.RevokeRolePermissionRequest{Id: custom.Msg.Role.Id, Permission: cadestrov1.Permission_PERMISSION_LIST_USERS}))
	require.Equal(t, connect.CodeNotFound, connect.CodeOf(err))
	require.EqualValues(t, 4, mustUser(t, service, ctx, target.ID).SessionVersion)
	_, err = service.SetRoleDescription(ctx, connect.NewRequest(&cadestrov1.SetRoleDescriptionRequest{Id: custom.Msg.Role.Id, Description: "Updated"}))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.EqualValues(t, 6, mustUser(t, service, ctx, target.ID).SessionVersion)
	...
	_, err = service.RenameRole(ctx, connect.NewRequest(&cadestrov1.RenameRoleRequest{Id: &cadestrov1.RoleId{Value: administratorsRoleID}, Name: "Administrators"}))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.EqualValues(t, 7, mustUser(t, service, ctx, target.ID).SessionVersion)
...
func TestEnableIdentityProviderValidatesBeforeMutation(t *testing.T) {
	...
	_, err = service.EnableIdentityProvider(ctx, connect.NewRequest(&cadestrov1.EnableIdentityProviderRequest{Id: &cadestrov1.IdentityProviderId{Value: provider.ID}}))
	require.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err))
	provider, err = service.store.Queries().GetIdentityProvider(ctx, provider.ID)
	require.NoError(t, err)
	require.False(t, provider.Enabled)
}
