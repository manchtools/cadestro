...
func TestApiToken_AuthorityChangesInvalidateAuthentication(t *testing.T) {
	...
	_, err = f.client.AssignRoleToUser(f.ctx(), authed(&cadestrov1.AssignRoleToUserRequest{UserId: &cadestrov1.UserId{Value: actor.ID}, RoleId: &cadestrov1.RoleId{Value: role}}, admin.Token))
	require.NoError(t, err)
	_, err = f.client.GetCurrentUser(f.ctx(), authed(&cadestrov1.GetCurrentUserRequest{}, created.Msg.Value))
	assert.Equal(t, connect.CodeUnauthenticated, connectCodeOf(t, err))

	subject := f.seedJITSubject()
	role = f.insertRole(allPermissionKeys())
	_, err = f.client.AssignRoleToUser(f.ctx(), authed(&cadestrov1.AssignRoleToUserRequest{UserId: &cadestrov1.UserId{Value: subject.ID}, RoleId: &cadestrov1.RoleId{Value: role}}, admin.Token))
	require.NoError(t, err)
	created, err = f.client.CreateApiToken(f.ctx(), authed(&cadestrov1.CreateApiTokenRequest{Name: "deleted", ExpiresAt: timestamppb.New(f.now.Add(time.Hour))}, f.mintToken(subject.ID, subject.Email)))
	require.NoError(t, err)
...
func TestApiToken_StaleAccessCannotMintAfterAuthorityInvalidation(t *testing.T) {
	...
	actor := f.seedActor(grant{Permissions: allPermissionKeys()})
	_, err := f.client.SetUserDisabled(f.ctx(), authed(&cadestrov1.SetUserDisabledRequest{Id: &cadestrov1.UserId{Value: actor.ID}, Disabled: true}, admin.Token))
	require.NoError(t, err)
